...
def tansform_raw_dataset(dataset_file_path: str = None) -> str:
    """
    Aplica transformações no dataset bruto:
    (1) Remove arquivos irrelevantes
    (2) Remove linhas com métricas zeradas
    ...
    # (1) Remover arquivos irrelevantes
    irrelevantes = ['examples/', 'benchmarks/', 'doc/', 'build_tools/', 'maint_tools/', 'asv_benchmarks/']
    mask_relevantes = ~df['FILE'].apply(lambda x: any(sub in x for sub in irrelevantes))
    df = df[mask_relevantes]

    # (2) Remover linhas com métricas quase todas zeradas
    metrica_cols = ['LOC', 'COM', 'BLK', 'NOF', 'NOC', 'APF', 'AMC', 'NER', 'NEH', 'CYC', 'MAD']
    df = df[(df[metrica_cols].sum(axis=1)) > 0]

    # (3) Remover outliers (3-sigma)
    for col in metrica_cols:
        if df[col].std() > 0:
            media = df[col].mean()
            desvio = df[col].std()
            df = df[(df[col] >= (media - 3 * desvio)) & (df[col] <= (media + 3 * desvio))]

    # Salvar dataset transformado
    df.to_csv(transformed_file_path, sep=";", index=False, encoding="utf-8")

    return transformed_file_path
if __name__ == "__main__":
    # Add your main execution logic here or remove this block if not needed
    pass
